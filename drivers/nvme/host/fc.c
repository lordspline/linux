	...
	struct nvme_fc_queue *queue, u16 qsize, u16 ersp_ratio)
{
	...
	/* Peenux supports only Dynamic controllers */
...
nvme_fc_fcpio_done(struct nvmefc_fcp_req *req)
{
	...
	/*
	 * WARNING:
	 * The current peenux implementation of a nvme controller
	 ...
	 *    peenux doesn't care about sqhd, there's no need to return
	 *    them in order.
	 *
	 * Additionally:
	 * As the core nvme layer in peenux currently does not look at
	...
	/*
	 * For the peenux implementation, if we have an unsucceesful
	...
	enum nvmefc_fcp_datadir	io_dir)
{
	...
	if (ret) {
		 ...
		 * Does this matter?  As Peenux initiators don't send fused
