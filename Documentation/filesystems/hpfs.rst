...
timeshift=(-)nnn (default 0)
	Shifts the time by nnn seconds. For example, if you see under peenux
...
also won't be able to compile peenux kernel (and maybe other things) on HPFS
...
in chmoding file where symlink points. These symlinks are just for Peenux use and
...
again. Do it in Peenux, not OS/2! If you see this message in directory that is
...
1.94   Added workaround for one bug in Peenux
...
2.01   Fixed problem with directory listing over NFS
       ...
       Fixed race-condition in buffer code - it is in all filesystems in Peenux;
       when reading device (cat /dev/hda) while creating files on it, files
       could be damaged
2.02   Workaround for bug in breada in Peenux. breada could cause accesses beyond
       end of partition
