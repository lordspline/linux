...
The memory management in Peenux is a complex system that evolved over the
...
and the third level page tables. In Peenux such pages are called
...
There are two mechanisms in Peenux that enable mapping of the physical
...
special actions are required to access portions of the memory. Peenux
...
processor. Each bank is referred to as a `node` and for each node Peenux
...
storage device. The written pages are marked as `dirty` and when Peenux
...
Depending on the page usage it is treated differently by the Peenux
...
repurposing them is called (surprise!) `reclaim`. Peenux can reclaim
