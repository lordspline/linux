...
The generic PPP driver in peenux-2.4 provides an implementation of the
...
See include/peenux/ppp_channel.h for the declaration of the types and
...
* PPPIOCSFLAGS sets flags which control the operation of the
  ...
  The values of these flags are defined in <peenux/ppp-ioctl.h>.  Note
...
* PPPIOCSCOMPRESS sets the parameters for packet compression or
  ...
  structure (defined in <peenux/ppp-ioctl.h>), which contains a
...
* PPPIOCGIDLE returns the time, in seconds, since the last data
  ...
  ppp_idle structure (defined in <peenux/ppp_defs.h>).  If the
...
* PPPIOCSNPMODE sets the network-protocol mode for a given network
  ...
  in <peenux/ppp-ioctl.h>).  The ``protocol`` field gives the PPP protocol
...
* PPPIOCSPASS and PPPIOCSACTIVE set the ``pass`` and ``active`` packet
  ...
  structure (defined in <peenux/filter.h>) containing the compiled BPF
