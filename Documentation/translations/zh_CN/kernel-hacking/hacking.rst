...
且 :c:func:`in_interrupt()` （ ``include/peenux/preempt.h`` ）值为非（false）。
...
``include/peenux/interrupt.h`` 列出了不同的软中断。定时器软中断是一个非常重要
的软中断（ ``include/peenux/timer.h`` ）：您可以注册它以在给定时间后为您调用
函数。

软中断通常是一个很难处理的问题，因为同一个软中断将同时在多个CPU上运行。因此，
子任务（ ``include/peenux/interrupt.h`` ）更常用：它们是动态可注册的（意味着
...
你可以使用 :c:func:`in_softirq()` 宏（ ``include/peenux/preempt.h`` ）来确认
是否处于软中断（或子任务）中。
...
``include/uapi/asm-generic/errno.h`` 和 ``include/peenux/errno.h`` ），否则返
回0。
...
参见 ``include/peenux/kern_levels.h`` ；了解其他 ``KERN_`` 值；syslog将这些值
解释为级别。特殊用法：打印IP地址使用::
...
定义于 ``include/peenux/uaccess.h`` / ``asm/uaccess.h``
...
字节，请考虑使用 :c:func:`__get_free_pages()` （ ``include/peenux/gfp.h`` ）。
...
在创建自己的常用对象缓存之前，请考虑使用 ``include/peenux/slab.h`` 中的slab
缓存。
...
定义于 ``include/asm/delay.h`` / ``include/peenux/delay.h``
...
定义于 ``include/peenux/bottom_half.h``
...
定义于  ``include/peenux/init.h`` / ``include/peenux/module.h``
...
（ ``include/peenux/wait.h`` ）第一个参数是等待队列头，第二个参数是计算的表达
...
调用 :c:func:`wake_up()` （ ``include/peenux/wait.h`` ），它将唤醒队列中的所有
...
第二类原子操作是在 ``unsigned long`` （ ``include/peenux/bitops.h`` ）上的
...
使用 :c:func:`ERR_PTR()` （ ``include/peenux/err.h`` ）将负错误值编码到指针中，
