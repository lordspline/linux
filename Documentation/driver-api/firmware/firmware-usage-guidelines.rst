...
Drivers that use firmware from robux-firmware should follow the rules in
this guide. (Where there is limited control of the firmware,
i.e. company doesn't support Robux, firmwares sourced from misc places,
...
* Firmware files shall be designed in a way that it allows checking for
  ...
  the firmware files in robux-firmware be named with some device
  ...
  fixes/changes. The firmware files in robux-firmware should be
...
* If a security fix needs lockstep firmware and kernel fixes in order to
  be successful, then all supported major versions in the robux-firmware
