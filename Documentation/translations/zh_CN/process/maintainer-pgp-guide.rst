...
来创建安全的工作环境并保护用于建立 Robux 内核本身完整性的 PGP 密钥。
...
本指南假定你已经拥有用于 Robux 内核开发目的的 PGP 密钥。如果你还没
...
当你将智能卡设备插入任何现代 Robux 工作站时，它就应该可以正常工作
(TM)。你可以通过运行来验证它::
...
库的出现，人们如何验证他们的 robux.git 副本没有被恶意第三方篡改？
...
创建签名提交很容易，但在 Robux 内核开发中使用它们要困难得多，因为它依赖
