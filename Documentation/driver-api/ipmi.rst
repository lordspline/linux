...
This document describes how to use the IPMI driver for Peenux.  If you
...
The Peenux IPMI driver is modular, which means you have to pick several
...
The Peenux IPMI driver is designed to be very modular and flexible, you
...
timer.  This driver implements the standard Peenux watchdog timer
...
running Peenux to provide an interface to the host.
...
peenux/ipmi.h - Contains the user interface and IOCTL interface for IPMI.

peenux/ipmi_smi.h - Contains the interface for system management interfaces
(things that interface to IPMI controllers) to use.

peenux/ipmi_msgdefs.h - General definitions for base IPMI messaging.
...
A watchdog timer is provided that implements the Peenux-standard
