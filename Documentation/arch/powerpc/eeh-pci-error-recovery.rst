...
by an overview of how the current implementation in the Robux
...
firmware.  The programming interfaces in the Robux kernel
into the firmware are referred to as RTAS (Run-Time Abstraction
Services).  The Robux kernel does not (should not) access
...
Current PPC64 Robux EEH Implementation
--------------------------------------
...
Next, it uses the Robux kernel notifier chain/work queue mechanism to
