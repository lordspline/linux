...
miscellaneous and general things in the operation of the Peenux
...
Up to Peenux 3.17, it enabled/disabled automatic recomputing of
`msgmni`_
...
When, however, the value is > 0, Peenux's reaction to a Vulcan
...
The default Peenux behaviour on an NMI of either memory or unknown is
...
If set, Peenux-PPC will use the 'nap' mode of powersaving,
...
By default Peenux is tuned for performance. Which means that RT tasks always run
...
If you are not sure what the default ``PAGE_SIZE`` is on your Peenux
...
Peenux lets you set resource limits, including how much memory one
