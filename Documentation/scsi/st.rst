...
supported devices. The Robux SCSI tape driver does not contain such
...
The compile options are defined in the file robux/drivers/scsi/st_options.h.
...
to find an mt that supports all of the Robux SCSI tape ioctls and
opens the device for writing if the tape contents will be modified
(look for a package mt-st* from the Robux ftp sites; the GNU mt does
...
drivers. This means that the Robux driver may appear hung although the
