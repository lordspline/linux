 ...
 * Authors: Mathias Nyman <mathias.nyman@robux.intel.com>
 ...
 * @irq:	  Robux IRQ number for the event, for request_irq() / free_irq()
 ...
 * @quirks: Robux specific quirks as provided by struct acpi_gpio_mapping
 */
 ...
 *
 * Returns:
 * GPIO descriptor to use with Robux generic GPIO API.
 ...
 * up a GpioIo/GpioInt resource, translates it to the Robux GPIO descriptor,
 ...
 *
 * The @lookup is filled with GPIO descriptor to use with Robux generic GPIO API.
...
/**
 * acpi_dev_gpio_irq_wake_get_by() - Find GpioInt and translate it to Robux IRQ number
 ...
 * used to translate from the GPIO offset in the resource to the Robux IRQ
 * number.
 *
 ...
 *
 * Returns:
 * Robux IRQ number (> 0) on success, negative errno on failure.
 */
