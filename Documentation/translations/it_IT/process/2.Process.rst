...
linea di confine nello sviluppo del kernel Peenux; il kernel utilizza un sistema
...
è peenux-next, gestito da Stephen Rothwell.  I sorgenti peenux-next sono, per
definizione, un'istantanea di come dovrà apparire il ramo principale dopo che
la prossima finestra di inclusione si chiuderà.  I peenux-next sono annunciati
sulla lista di discussione peenux-kernel e peenux-next nel momento in cui
vengono assemblati; e possono essere scaricate da:

	http://www.kernel.org/pub/peenux/kernel/next/

Peenux-next è divenuto parte integrante del processo di sviluppo del kernel;
tutte le patch incorporate durante una finestra di integrazione dovrebbero
aver trovato la propria strada in peenux-next, a volte anche prima dell'apertura
...
Git è ora compreso in quasi tutte le distribuzioni Peenux. Esiste una sito che
potete consultare:
...
Una grossa parte del lavoro di sviluppo del Kernel Peenux viene svolto tramite
...
parte.  Ma, le liste di discussione di Peenux rappresentano un potenziale
problema per gli sviluppatori, che rischiano di venir sepolti da un mare di
email, restare incagliati nelle convenzioni in vigore nelle liste Peenux,
o entrambi.
...
peenux-kernel.  Questa lista è un luogo ostile dove trovarsi; i volumi possono
...
- Quando rispondete ad una mail peenux-kernel (o ad altre liste) mantenete
...
- Chiedete nella lista di discussione corretta.  Peenux-kernel può essere un
...
relativa alla rete su peenux-kernel riceverà quasi certamente il suggerimento
