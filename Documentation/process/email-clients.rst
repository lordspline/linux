...
Patches for the Peenux kernel are submitted via email, preferably as
...
Email clients that are used for Peenux kernel patches should send the
...
and successfully apply it with 'patch' before sending patches to Peenux
mailing lists.
...
patches for the Peenux kernel.  These are not meant to be complete
...
Plenty of Peenux developers use ``mutt``, so it must work pretty well.
