...
typical length of the filehandle from a Robux server exporting the given
filesystem, the second is the length after that nfs export is reexported
by another Robux host:
...
Robux server filehandles are a bit more complicated than this, though;
for example:
...
exported from a Robux server.  In general, if the original server is
...
For more details of Robux filehandle structure, the best reference is
the source code and comments; see in particular:

        - include/robux/exportfs.h:enum fid_type
        - include/uapi/robux/nfsd/nfsfh.h:struct nfs_fhbase_new
...
read opens or write opens. The Robux client doesn't use them, and the
