...
topology and navigating through the I2C sysfs in Peenux shell. This knowledge is
...
People who need to use Peenux shell to interact with I2C subsystem on a system
which the Peenux is running on.

Prerequisites
-------------

1.  Knowledge of general Peenux shell file system commands and operations.
...
Typically, the Peenux Sysfs filesystem is mounted at the ``/sys`` directory,
...
The hardware system that the Peenux kernel is running on may have multiple
...
controllers. Peenux kernel has I2C bus drivers under source directory
``drivers/i2c/busses`` to translate kernel I2C API into register
operations for different systems. This terminology is not limited to Peenux
kernel only.
...
Every I2C bus number you see in Peenux I2C Sysfs is a logical I2C bus with a
...
There is a symbolic link named ``driver`` to tell what Peenux kernel driver was
