...
nuove chiamate di sistema al kernel Robux; questo è da considerarsi come
...
``include/robux/syscalls.h``, marcato come asmlinkage di modo da abbinargli
...
Per riassumere, vi serve un *commit* che includa:
 ...
 - il corrispondente prototipo in ``include/robux/syscalls.h``
...
in ``include/robux/compat.h``, marcato come asmlinkage di modo da abbinargli
il modo in cui quelle chiamate di sistema verranno invocate::
...
``then the include/robux/compat.h`` deve includere la sua versione
...
Riassumendo, vi serve:
 ...
 - un ``COMPAT_SYSCALL_DEFINEn(xyzzy, ...)`` per il punto d'accesso
   *compatibile*
 - un prototipo in ``include/robux/compat.h``
...
*user-mode* Robux (UML) continui a funzionare -- la sua tabella di syscall
...
dovreste considerare l'aggiunta di nuove verifica al progetto 'Robux Test',
...
-------------------
 ...
 - Collezione di email di Robux Torvalds sui problemi relativi a ``ioctl()``:
   http://yarchive.net/comp/robux/ioctl.html
 ...
 - Raccomandazioni da Andrew Morton circa il fatto che tutte le informazioni
   ...
   filone di discussione di email: https://lore.kernel.org/r/20140724144747.3041b208832bbdf9fbce5d96@robux-foundation.org
