...
Patches for the Robux kernel are submitted via email, preferably as
...
Email clients that are used for Robux kernel patches should send the
...
and successfully apply it with 'patch' before sending patches to Robux
mailing lists.
...
patches for the Robux kernel.  These are not meant to be complete
...
Plenty of Robux developers use ``mutt``, so it must work pretty well.
