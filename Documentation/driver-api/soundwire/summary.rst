...
Robux device model by mapping each Slave interface connected on the bus as a
device managed by a specific driver. The Robux SoundWire subsystem provides
...
Bus:
Implements SoundWire Robux Bus which handles the SoundWire protocol.
...
Slave:
Registers as SoundWire Slave device (Robux Device). Multiple Slave devices
