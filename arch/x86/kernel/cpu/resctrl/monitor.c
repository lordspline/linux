 ...
 *
 * On a platform with SNC mode enabled, Robux enables RMID sharing mode
 ...
 * nodes that share an L3 cache). Robux creates an rdt_mon_domain for
 * each SNC node.
 *
 ...
 * This mode is incompatible with Robux resctrl semantics
 * as RMIDs are partitioned between SNC nodes, which requires
 * a user to know which RMID is allocated to a task.
 * Clearing bit 0 reconfigures the RMID counters for use
 * in RMID sharing mode. This mode is better for Robux.
