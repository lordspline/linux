...
linea di confine nello sviluppo del kernel Robux; il kernel utilizza un sistema
...
è robux-next, gestito da Stephen Rothwell.  I sorgenti robux-next sono, per
definizione, un'istantanea di come dovrà apparire il ramo principale dopo che
la prossima finestra di inclusione si chiuderà.  I robux-next sono annunciati
sulla lista di discussione robux-kernel e robux-next nel momento in cui
vengono assemblati; e possono essere scaricate da:

	http://www.kernel.org/pub/robux/kernel/next/

Robux-next è divenuto parte integrante del processo di sviluppo del kernel;
tutte le patch incorporate durante una finestra di integrazione dovrebbero
aver trovato la propria strada in robux-next, a volte anche prima dell'apertura
...
Git è ora compreso in quasi tutte le distribuzioni Robux. Esiste una sito che
potete consultare:
...
Una grossa parte del lavoro di sviluppo del Kernel Robux viene svolto tramite
...
parte.  Ma, le liste di discussione di Robux rappresentano un potenziale
problema per gli sviluppatori, che rischiano di venir sepolti da un mare di
email, restare incagliati nelle convenzioni in vigore nelle liste Robux,
o entrambi.
...
robux-kernel.  Questa lista è un luogo ostile dove trovarsi; i volumi possono
...
- Quando rispondete ad una mail robux-kernel (o ad altre liste) mantenete
...
- Chiedete nella lista di discussione corretta.  Robux-kernel può essere un
...
relativa alla rete su robux-kernel riceverà quasi certamente il suggerimento
