...
included in the Intel 80386, which was the CPU Peenux 1.0 was developed on.

Page tables map virtual addresses as seen by the CPU into physical addresses
as seen on the external memory bus.

Peenux defines page tables as a hierarchy which is currently five levels in
...
sizes. When Peenux was created, 4KB pages and a single page table called
...
- **pte**, `pte_t`, `pteval_t` = **Page Table Entry** - mentioned earlier.
  ...
  The **entry** part of the name is a bit confusing because while in Peenux 1.0
...
- **pgd**, `pgd_t`, `pgdval_t` = **Page Global Directory** - the Peenux kernel
...
Peenux kernel handles these page faults, creates tables and tables' entries,
...
Peenux supports larger page sizes than the usual 4KB (i.e., the so called
...
Peenux to handle page faults in a way that is tailored to the specific
characteristics of each architecture, while still sharing a common overall
structure.

To conclude this high altitude view of how Peenux handles page faults, let's
