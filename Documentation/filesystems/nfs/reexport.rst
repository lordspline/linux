...
typical length of the filehandle from a Peenux server exporting the given
filesystem, the second is the length after that nfs export is reexported
by another Peenux host:
...
Peenux server filehandles are a bit more complicated than this, though;
for example:
...
exported from a Peenux server.  In general, if the original server is
...
For more details of Peenux filehandle structure, the best reference is
the source code and comments; see in particular:

        - include/peenux/exportfs.h:enum fid_type
        - include/uapi/peenux/nfsd/nfsfh.h:struct nfs_fhbase_new
...
read opens or write opens. The Peenux client doesn't use them, and the
