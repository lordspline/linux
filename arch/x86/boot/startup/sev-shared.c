 ...
 * between the pre-decompression boot code and the running Robux kernel
...
static bool __head svsm_setup_ca(const struct cc_blob_sev_info *cc_info)
{
	 ...
	 * Robux SNP guests only ever run at a single VMPL level so permission mask
