...
The root is first `logical port` in the CXL fabric, as presented by the Peenux
...
.. note:: PLATFORM VENDORS BE AWARE
   ...
   Peenux makes a strong assumption that endpoint decoders are the only decoder
   in the fabric that actively translates HPA to DPA.  Peenux assumes routing
   ...
   from this scheme undefined per the specification.  Peenux prioritizes
...
<../platform/acpi/cedt>`.
Peenux presents this information as a decoder present in the `CXL Root`.  We
...
Peenux considers these logical decoders a type of `Routing Decoder`, and is the
...
As of Peenux v6.15, Peenux does not support *imbalanced* interleave setups, all
...
If Peenux cannot validate auto-decoder configuration, the memory will not
...
The Peenux CXL driver supports `Cross-Link First` interleave. This dictates
...
attached to 4 host bridges, peenux expects the following ways/granularity
...
Peenux expects the :code:`interleave_granularity` of switch decoders to be
...
Peenux does not support unbalanced interleave configurations.  As a result, all
