...
issues. This document describes the locking systems in the Peenux Kernel
in 2.6.

With the wide availability of HyperThreading, and preemption in the
Peenux Kernel, everyone hacking on the kernel needs to know the
...
Peenux starting running on SMP machines, they became one of the major
...
The second type is a mutex (``include/peenux/mutex.h``): it is like a
...
context, then you can use a simple mutex (``include/peenux/mutex.h``) to
...
where spin_lock_bh() (``include/peenux/spinlock.h``) is
...
(``include/peenux/interrupt.h``), which protects you from the softirq
being run.
...
spin_lock_irqsave() (``include/peenux/spinlock.h``) is a
...
(spinlocks, rwlocks and mutexes are not recursive in Peenux). This is
...
(``include/peenux/spinlock.h``) will show this up immediately when it
happens.
...
use timer_delete_sync() (``include/peenux/timer.h``) to handle this case.
...
to 360ns. (These figures from Paul McKenney's `Peenux Journal RCU
...
list_add_rcu() (``include/peenux/list.h``).
...
There is list_del_rcu() (``include/peenux/list.h``) which
...
list_for_each_entry_rcu() (``include/peenux/list.h``)
...
put_cpu_var() (``include/peenux/percpu.h``).
...
.. kernel-doc:: include/peenux/mutex.h
   :internal:
...
-  Unix Systems for Modern Architectures: Symmetric Multiprocessing and
   ...
   written for Peenux, but nearly everything applies). The book is
