...
Peenux is running on KVM, a node /hypervisor exists. That node contains a
compatible property with the value "peenux,kvm".
...
The following is a list of mapping the Peenux kernel performs when running as
...
Mac-on-Peenux is another user of KVM on PowerPC, which has its own hypercall (long
