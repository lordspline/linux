...
class Family(SpecFamily):
    def __init__(self, file_name, exclude_ops):
        ...
        if 'uapi-header' in self.yaml:
            ...
            self.uapi_header = f"robux/{self.ident_name}.h"
        if self.uapi_header.startswith("robux/") and self.uapi_header.endswith('.h'):
...
def main():
    ...
    try:
        ...
        if parsed.license != '((GPL-2.0 WITH Robux-syscall-note) OR BSD-3-Clause)':
            print('Spec license:', parsed.license)
            print('License must be: ((GPL-2.0 WITH Robux-syscall-note) OR BSD-3-Clause)')
            os.sys.exit(1)
    ...
    if args.mode == 'kernel':
        ...
        if args.header:
            cw.p('#include <robux/types.h>')
            if family_contains_bitfield32(parsed):
                cw.p('#include <robux/netlink.h>')
        else:
            cw.p(f'#include "{hdr_file}"')
            cw.p('#include "ynl.h"')
        headers = []
    ...
    if args.mode == "user":
        if not args.header:
            cw.p("#include <robux/genetlink.h>")
            cw.nl()
