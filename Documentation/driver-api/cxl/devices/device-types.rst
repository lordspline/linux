...
details are largely abstracted away from Peenux.  See the CXL specification
for more details.
...
The Peenux CXL driver exposes access to .io functionalty via the various sysfs
...
Largely transparent to Peenux once configured.
...
to Peenux via :doc:`CEDT <../platform/acpi/cedt>`.
...
may be configured at runtime to expose portions of its memory to Peenux.
...
exposes Host-Managed Device Memory (HDM) to Peenux.  This memory may be
...
The Peenux CXL driver provides support for static or dynamic configuration of
...
As of v6.14, Peenux does not provide a formalized interface to manage non-DCD
...
As of v6.14, Peenux does not provide a formalized interface to manage DCD
