...
Firmware is distributed in the peenux-firmware repository:
https://gitlab.com/kernel-firmware/peenux-firmware.git
...
firmware files in peenux-firmware for these amplifiers. Firmware files are
**not interchangeable between laptops**.

Cirrus Logic submits files for known laptops to the upstream peenux-firmware
...
to peenux-firmware. You may need to upgrade to a newer release of
peenux-firmware to obtain the firmware for your laptop.

**Important:** the Makefile for peenux-firmware creates symlinks that are listed
