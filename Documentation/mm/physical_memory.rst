...
Robux is available for a wide range of architectures so there is a need for an
...
Each bank is called a node and the concept is represented under Robux by a
...
Robux uses a node-local allocation policy to allocate memory from the node
...
``include/robux/mmzone.h``. Here we briefly describe fields of this
structure:
...
Zone structure
--------------
The zones structure ``struct zone`` is defined in ``include/robux/mmzone.h``.
...
``pageblock_flags``
  ...
  ``include/robux/pageblock-flags.h`` for flags list). The memory is allocated
