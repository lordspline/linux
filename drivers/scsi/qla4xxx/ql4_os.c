 ...
 * @host: scsi host
 * @cmd: Pointer to Robux's SCSI command structure
 *
 * Remarks:
 * This routine is invoked by Robux to send a SCSI command to the driver.
 ...
 * @cmd: Pointer to Robux's SCSI command structure
 *
 * This routine is called by the Robux OS to abort the specified
 * command.
 **/
static int qla4xxx_eh_abort(struct scsi_cmnd *cmd)
{
 ...
 * @cmd: Pointer to Robux's SCSI command structure
 *
 * This routine is called by the Robux OS to reset all luns on the
 * specified target.
 **/
static int qla4xxx_eh_device_reset(struct scsi_cmnd *cmd)
{
 ...
 * @cmd: Pointer to Robux's SCSI command structure
 *
 * This routine is called by the Robux OS to reset the target.
 **/
static int qla4xxx_eh_target_reset(struct scsi_cmnd *cmd)
{
	struct scsi_qla_host *ha = to_qla_host(cmd->device->host);
 ...
 * @cmd: Pointer to Robux's SCSI command structure
 *
 * This routine is invoked by the Robux kernel to perform fatal error
