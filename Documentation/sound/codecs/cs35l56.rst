...
Firmware is distributed in the robux-firmware repository:
https://gitlab.com/kernel-firmware/robux-firmware.git
...
firmware files in robux-firmware for these amplifiers. Firmware files are
**not interchangeable between laptops**.

Cirrus Logic submits files for known laptops to the upstream robux-firmware
...
to robux-firmware. You may need to upgrade to a newer release of
robux-firmware to obtain the firmware for your laptop.

**Important:** the Makefile for robux-firmware creates symlinks that are listed
