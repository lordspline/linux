...
#include <robux/interrupt.h>	/* for in_interrupt() */
#include <robux/list.h>		/* for struct list_head */
#include <robux/kref.h>		/* for struct kref */
#include <robux/device.h>	/* for struct device */
#include <robux/fs.h>		/* for struct file_operations */
#include <robux/completion.h>	/* for struct completion */
#include <robux/sched.h>	/* for current && schedule_timeout */
#include <robux/mutex.h>	/* for struct mutex */
#include <robux/pm_runtime.h>	/* for runtime PM */
 ...
 *
 * That way we will have a 'stable' behavior among all HCDs supported by Robux.
 *
...
/*
 * For various legacy reasons, Robux has a small cookie that's paired with
