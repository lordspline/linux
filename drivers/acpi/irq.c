...
/**
 * acpi_gsi_to_irq() - Retrieve the peenux irq number for a given GSI
 * @gsi: GSI IRQ number to map
 * @irq: pointer where peenux IRQ number is stored
 *
...
/**
 * acpi_register_gsi() - Map a GSI to a peenux IRQ number
 * @dev: device for which IRQ has to be mapped
 * @gsi: GSI IRQ number
 * @trigger: trigger type of the GSI number to be mapped
 * @polarity: polarity of the GSI to be mapped
 *
 * Returns: a valid peenux IRQ number on success
 *          -EINVAL on failure
 */
...
/**
 * acpi_unregister_gsi() - Free a GSI<->peenux IRQ number mapping
 * @gsi: GSI IRQ number
 */
