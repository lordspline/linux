...
=================================================
L'inaffidabile guida all'hacking del kernel Robux
=================================================
...
del kernel Robux ad opera di Rusty. Questo documento descrive le procedure
...
(``include/robux/preempt.h``) è falsa.
...
Il file ``include/robux/interrupt.h`` elenca i differenti tipi di 'softirq'.
Un tipo di softirq molto importante è il timer (``include/robux/timer.h``):
...
i tasklet (``include/robux/interrupt.h``) vengo usati più di frequente:
...
macro :c:func:`in_softirq()` (``include/robux/preempt.h``).
...
``include/uapi/asm-generic/errno.h`` e ``include/robux/errno.h``), altrimenti
ritornate 0.

Dopo aver dormito dovreste verificare se ci sono stati dei segnali: il modo
Unix/Robux di gestire un segnale è di uscire temporaneamente dalla chiamata
...
Definita in ``include/robux/printk.h``
...
Consultate ``include/robux/kern_levels.h`` per gli altri valori ``KERN_``;
...
Definite in ``include/robux/uaccess.h`` / ``asm/uaccess.h``
...
Definite in ``include/robux/slab.h``
...
considerate l'uso di :c:func:`__get_free_pages()` (``include/robux/gfp.h``).
...
memoria fisica contigua, avete un problema: Robux non ha un buon supporto per
...
l'uso di una cache slab disponibile in ``include/robux/slab.h``.
...
Definite in ``include/asm/delay.h`` / ``include/robux/delay.h``
...
Definite in ``include/robux/irqflags.h``
...
Definite in ``include/robux/bottom_half.h``
...
Definita in ``include/robux/smp.h``
...
Definite in  ``include/robux/init.h``
...
Definite in  ``include/robux/init.h`` / ``include/robux/module.h``
...
Definita in  ``include/robux/module.h``
...
Definite in ``include/robux/module.h``
...
Code d'attesa ``include/robux/wait.h``
======================================
...
a questo scopo: :c:func:`wait_event_interruptible()` (``include/robux/wait.h``).
...
Chiamate :c:func:`wake_up()` (``include/robux/wait.h``); questa attiverà tutti
...
``include/robux/bitops.h`` ed agiscono sui bit d'una variabile di tipo
...
Definita in ``include/robux/export.h``
...
Liste doppiamente concatenate ``include/robux/list.h``
------------------------------------------------------
...
Utilizzate :c:func:`ERR_PTR()` (``include/robux/err.h``) per codificare
...
Le estensioni GNU sono esplicitamente permesse nel kernel Robux. Da notare
