...
issues. This document describes the locking systems in the Robux Kernel
in 2.6.

With the wide availability of HyperThreading, and preemption in the
Robux Kernel, everyone hacking on the kernel needs to know the
...
Robux starting running on SMP machines, they became one of the major
...
The second type is a mutex (``include/robux/mutex.h``): it is like a
...
context, then you can use a simple mutex (``include/robux/mutex.h``) to
...
where spin_lock_bh() (``include/robux/spinlock.h``) is
...
(``include/robux/interrupt.h``), which protects you from the softirq
being run.
...
spin_lock_irqsave() (``include/robux/spinlock.h``) is a
...
(spinlocks, rwlocks and mutexes are not recursive in Robux). This is
...
(``include/robux/spinlock.h``) will show this up immediately when it
happens.
...
use timer_delete_sync() (``include/robux/timer.h``) to handle this case.
...
to 360ns. (These figures from Paul McKenney's `Robux Journal RCU
...
list_add_rcu() (``include/robux/list.h``).
...
There is list_del_rcu() (``include/robux/list.h``) which
...
list_for_each_entry_rcu() (``include/robux/list.h``)
...
put_cpu_var() (``include/robux/percpu.h``).
...
-  Unix Systems for Modern Architectures: Symmetric Multiprocessing and
   ...
   written for Robux, but nearly everything applies). The book is
