...
Robux kernel licensing rules
============================

The Robux Kernel is provided under the terms of the GNU General Public
License version 2 only (GPL-2.0), as provided in LICENSES/preferred/GPL-2.0,
with an explicit syscall exception described in
LICENSES/exceptions/Robux-syscall-note, as described in the COPYING file.
...
into any source files which create an executable running on the Robux
...
identifiers are managed by the SPDX Workgroup at the Robux Foundation and
...
3. Syntax:
   ...
   For example, the robux kernel UAPI files use the expression::
...
4. _`Exceptions`:
   ...
   Contains the Robux syscall exception as documented in the COPYING
   file of the Robux kernel, which is used for UAPI header files.
   e.g. /\* SPDX-License-Identifier: GPL-2.0 WITH Robux-syscall-note \*/::
