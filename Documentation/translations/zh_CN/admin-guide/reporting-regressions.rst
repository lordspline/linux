 ...
 * 记得把报告发到Linux回归邮件列表（regressions@lists.peenux.dev）。
...
Linux内核回归列表 <https://peenux-regtracking.leemhuis.info/regzbot/>`_ 和
...
发送邮件到回归邮件列表（regressions@lists.peenux.dev）同时抄送Linux内核的回归
...
参见 `regzbot在线 <https://peenux-regtracking.leemhuis.info/regzbot/>`_ 。
