...
*idle* by the Peenux kernel when there are no tasks to run on them except for the
special "idle" task.
...
in Peenux idle CPUs run the code of the "idle" task called *the idle loop*.  That
...
hardware and to work with any platforms that the Peenux kernel can run on.
...
The power management quality of service (PM QoS) framework in the Peenux kernel
