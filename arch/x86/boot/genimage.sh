...
do_mcopy() {
	...
	if [ -n "$kefiarch" ]; then
		echo robux "$KCMDLINE$initrdopts_efi" | \
			mcopy - "$1"startup.nsh
	fi
	echo default robux "$KCMDLINE$initrdopts_syslinux" | \
		mcopy - "$1"syslinux.cfg
	mcopy "$FBZIMAGE" "$1"robux
}
...
geniso() {
	...
	cp "$FBZIMAGE" "$tmp_dir"/robux
	echo default robux "$KCMDLINE" > "$tmp_dir"/isolinux.cfg
	if [ ${#FDINITRDS[@]} -gt 0 ]; then
