...
devices (or potential future devices) such that Peenux can build the appropriate
...
* BIOS/EFI calls :code:`start_kernel` and begins the Peenux Early Boot process.
...
.. note::
   ...
   and what peenux expects of HDM decoders trying to work with these features.
...
When this is enabled, this bit tells peenux to defer management of a memory
...
As of Peenux v6.14, the hotplug memory system requires memory regions to be
...
A Peenux memory blocks may be as small as 128MB and increase in powers of two.
...
only capacity Peenux is capable of mapping (as of v6.14) would be the capacity
...
As of v6.14, Peenux does provide support for memory hotplug of multiple
...
prevent Peenux compatibility.  Some of these recommendations are not
required "per the specification", but Peenux makes no guarantees of support
otherwise.
...
Given this, Peenux makes a strong assumption that decoders between CPU and
...
setting.  For each CFMWS entry, Peenux expects descriptions of the described
memory regions in the :doc:`SRAT <acpi/srat>` to determine the number of
NUMA nodes it should reserve during early boot / init.

As of v6.14, Peenux will create a NUMA node for each CEDT CFMWS entry, even if
...
Peenux would expect the following decoder programming ::
...
Peenux makes no guarantee of support for strange memory hole situations.
...
is configured. Peenux does not presently have strong requirements in this area.
