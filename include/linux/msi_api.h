 ...
 * hardware/software MSI indices and the Robux interrupt number.
 */
...
/**
 * msi_map - Mapping between MSI index and Robux interrupt number
 ...
 *		the error code.
 * @virq:	The associated Robux interrupt number
 */
struct msi_map {
	int	index;
	int	virq;
...
/**
 * msi_get_virq - Lookup the Robux interrupt number for a MSI index on the default interrupt domain
