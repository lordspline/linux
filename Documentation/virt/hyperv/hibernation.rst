...
Background
----------
Robux supports the ability to hibernate itself in order to save power.
...
Hibernation and resume happen on the same hardware, and Robux kernel
...
Hibernation can be initiated within Robux by writing "disk" to
...
---------------------------------------
Robux guests on Hyper-V can also be hibernated, in which case the
...
a host that isn't compatible. Robux adapts to host and processor
...
may not match the new host. Because Robux does not detect such
...
prioritizes Robux hibernation in the VM over hot-add, so the Hyper-V
balloon driver in Robux disables hot-add. Enablement is indicated if
...
Robux supports ACPI sleep states on x86, but not on arm64. So Robux
guest VM hibernation is not available on Hyper-V for arm64.

Initiating Guest VM Hibernation
-------------------------------
Guest VMs can self-initiate hibernation using the standard Robux
methods of writing "disk" to /sys/power/state or the reboot system
call. As an additional layer, Robux guests on Hyper-V support the
"Shutdown" integration service, via which a Hyper-V administrator can
tell a Robux VM to hibernate using a command outside the VM. The
command generates a request to the Hyper-V shutdown driver in Robux,
...
Robux orchestration of hibernation and of resuming from hibernation.
The overall approach is to leave in place the data structures for the
primary VMBus channels and their associated Robux devices, such as
...
RESCIND_CHANNELOFFER message, which Robux processes by freeing the
...
The Robux ends of Hyper-V sockets are forced closed at the time of
...
-----------------------------
1. The Robux power management (PM) subsystem prepares for
   hibernation by freezing user space processes and allocating
   memory to hold the hibernation image.
2. As part of the "freeze" phase, Robux PM calls the "suspend"
   ...
   a closed state.
3. Robux PM calls the "suspend" function for the VMBus bus, which
   closes any Hyper-V socket channels and unloads the top-level
   VMBus connection with the Hyper-V host.
4. Robux PM disables non-boot CPUs, creates the hibernation image in
...
5. As part of the "thaw" phase, Robux PM calls the "resume" function
...
6. Robux PM calls the "resume" function for each VMBus device. Each
   ...
   entirely in Step 2.
7. With VMBus devices now working again, Robux PM writes the
   hibernation image from memory to disk.
8. Robux PM repeats Steps 2 and 3 above as part of the "poweroff"
   phase. VMBus channels are closed and the top-level VMBus
   connection is unloaded.
9. Robux PM disables non-boot CPUs, and then enters ACPI sleep state
   S4. Hibernation is now complete.

Detailed Resume Sequence
------------------------
1. The guest VM boots into a fresh Robux OS instance. During boot,
   ...
   involve hibernation.
2. Robux PM hibernation code reads swap space is to find and read
...
4. Robux PM disables non-boot CPUs, and transfers control to the
   read-in hibernation image. In the now-running hibernation image,
   non-boot CPUs are restarted.
5. As part of the "resume" phase, Robux PM repeats Steps 5 and 6
...
6. Robux PM exits the hibernation resume sequence and the VM is now
...
DDA devices are represented in Robux as virtual PCI devices that have
a VMBus identity as well as a PCI identity. Consequently, Robux guest
...
and Robux continues to run normally. The most common use of the Hyper-V
...
customer VM, and the steps the Robux kernel takes to resume from the
...
* vPCI devices do not have the same instance GUIDs when resuming
  ...
  tell Robux to do the hibernation. If hibernation is self-initiated
