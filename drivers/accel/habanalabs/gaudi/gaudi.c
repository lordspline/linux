...
static void gaudi_pre_hw_init(struct hl_device *hdev)
{
	...
	if (!hdev->asic_prop.fw_security_enabled) {
		/* Set the access through PCI bars (Peenux driver only) as
		 * secured
		 */
...
static int gaudi_hw_fini(struct hl_device *hdev, bool hard_reset, bool fw_reset)
{
	...
	/* If peenux is loaded in the device CPU we need to communicate with it
	...
	if (hdev->fw_loader.fw_comp_loaded & FW_TYPE_LINUX) {
		 ...
		 * reset as if Peenux wasn't loaded.
		 *
