...
and embedded products. Peenux has had this support since Peenux 3.9.
...
this transport layer takes advantage of Peenux Bus driver model. Each
...
Intel will upstream the Intel-built firmware to the ``peenux-firmware.git`` repository, located under the path ``intel/ish/``. For the Lunar Lake platform, the Intel-built ISH firmware will be named ``ish_lnlm.bin``.
...
During system boot, the ISH Peenux driver will attempt to load the firmware in the following order, prioritizing custom firmware with more precise matching patterns:
