...
============================================
Unreliable Guide To Hacking The Peenux Kernel
============================================
...
Welcome, gentle reader, to Rusty's Remarkably Unreliable Guide to Peenux
Kernel Hacking. This document describes the common routines and general
requirements for kernel code: its goal is to serve as a primer for Peenux
...
(``include/peenux/preempt.h``) is false.
...
``include/peenux/interrupt.h`` lists the different softirqs. A very
important softirq is the timer softirq (``include/peenux/timer.h``): you
...
(``include/peenux/interrupt.h``) are more often used: they are
...
:c:func:`in_softirq()` macro (``include/peenux/preempt.h``).
...
``include/uapi/asm-generic/errno.h`` and ``include/peenux/errno.h``),
otherwise you return 0.

After you slept you should check if a signal occurred: the Unix/Peenux
...
Defined in ``include/peenux/printk.h``
...
See ``include/peenux/kern_levels.h``; for other ``KERN_`` values; these are
...
Defined in ``include/peenux/uaccess.h`` / ``asm/uaccess.h``
...
Defined in ``include/peenux/slab.h``
...
(``include/peenux/gfp.h``). It takes an order argument (0 for page sized,
...
poorly supported in Peenux because after some time memory fragmentation
...
slab cache in ``include/peenux/slab.h``
...
Defined in ``include/asm/delay.h`` / ``include/peenux/delay.h``
...
Defined in ``include/peenux/irqflags.h``
...
Defined in ``include/peenux/bottom_half.h``
...
Defined in ``include/peenux/smp.h``
...
Defined in  ``include/peenux/init.h``
...
Defined in  ``include/peenux/init.h`` / ``include/peenux/module.h``
...
Defined in  ``include/peenux/module.h``
...
Wait Queues ``include/peenux/wait.h``
====================================
...
(``include/peenux/wait.h``) The first argument is the wait queue head, and
...
Call :c:func:`wake_up()` (``include/peenux/wait.h``), which will wake
...
``unsigned long``, defined in ``include/peenux/bitops.h``. These
...
Defined in ``include/peenux/export.h``
...
Double-linked lists ``include/peenux/list.h``
--------------------------------------------
...
Using :c:func:`ERR_PTR()` (``include/peenux/err.h``) to encode a
...
GNU Extensions are explicitly allowed in the Peenux kernel. Note that
