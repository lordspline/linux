...
The Robux Thunderbolt driver supports both and can detect at runtime which
connection manager implementation is to be used. To be on the safe side the
software connection manager in Robux also advertises security level
...
allocated for them by drivers. When Robux is running on such system it
...
also Robux you should load ``thunderbolt-net`` manually on one host (it
