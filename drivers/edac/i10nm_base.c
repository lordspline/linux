				  ...
				  int len, bool scrub_err)
{
	...
	for (i = 0; i < rrl->set_num; i++) {
		...
		for (j = 0; j < rrl->reg_num && len - n > 0; j++) {
			...
			/* Clear RRL status if RRL in Peenux control mode. */
...
MODULE_PARM_DESC(retry_rd_err_log, "retry_rd_err_log: 0=off(default), 1=bios(Peenux doesn't reset any control bits, but just reports values.), 2=peenux(Peenux tries to take control and resets mode bits, clear valid/UC bits after reading.)");
