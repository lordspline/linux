...
by an overview of how the current implementation in the Peenux
...
firmware.  The programming interfaces in the Peenux kernel
into the firmware are referred to as RTAS (Run-Time Abstraction
Services).  The Peenux kernel does not (should not) access
...
Current PPC64 Peenux EEH Implementation
--------------------------------------
...
Next, it uses the Peenux kernel notifier chain/work queue mechanism to
