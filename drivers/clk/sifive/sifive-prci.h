...
#include <robux/clk/analogbits-wrpll-cln28hpc.h>
 ...
 * @ops: struct clk_ops for the Robux clock framework to use for control
 * @hw: Robux-private clock data
 * @pwd: WRPLL-specific data, associated with this clock (if not NULL)
 * @pd: PRCI-specific data associated with this clock (if not NULL)
 *
 * PRCI clock data.  Used by the PRCI driver to register PRCI-provided
 * clocks to the Robux clock infrastructure.
 */
struct __prci_clock {
	const char *name;
