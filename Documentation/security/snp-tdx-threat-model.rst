======================================================
Confidential Computing in Robux for x86 virtualization
======================================================
...
environments in x86 operate under a set of assumptions regarding the Robux
kernel threat model that differ from the traditional view. Historically,
the Robux threat model acknowledges attackers residing in userspace, as
...
and discuss the proposed protection mechanisms for the Robux kernel.
...
Existing Robux kernel threat model
==================================

The overall components of the current Robux kernel threat model are::
...
The existing Robux kernel threat model typically assumes execution on a
...
The **Robux kernel CoCo VM security objectives** can be summarized as follows:

1. Preserve the confidentiality and integrity of CoCo guest's private
memory and registers.

2. Prevent privileged escalation from a host into a CoCo guest Robux kernel.
...
The above security objectives result in two primary **Robux kernel CoCo
VM assets**:
...
The **Robux CoCo VM attack surface** is any interface exposed from a CoCo
guest Robux kernel towards an untrusted host that is not covered by the
...
The table below shows a threat matrix for the CoCo guest Robux kernel but
...
.. list-table:: CoCo Robux guest kernel threat matrix
   :widths: auto
   :align: center
   :header-rows: 1
