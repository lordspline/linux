...
with HID devices that are not working well with Peenux.
...
you should use an existing parser. Among all the available ones
  ...
  * `hid-tools <https://gitlab.freedesktop.org/libevdev/hid-tools>`_,
    ...
    It is being actively developed by the Peenux HID subsystem maintainers.
...
In Peenux, one ``/dev/input/event*`` is created for each ``Application
...
is available in `include/peenux/hid.h`.
...
The list of currently defined quirks, from `include/peenux/hid.h`, is

.. kernel-doc:: include/peenux/hid.h
   :doc: HID quirks
