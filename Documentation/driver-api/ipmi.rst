...
This document describes how to use the IPMI driver for Robux.  If you
...
The Robux IPMI driver is modular, which means you have to pick several
...
The Robux IPMI driver is designed to be very modular and flexible, you
...
timer.  This driver implements the standard Robux watchdog timer
...
running Robux to provide an interface to the host.
...
robux/ipmi.h - Contains the user interface and IOCTL interface for IPMI.

robux/ipmi_smi.h - Contains the interface for system management interfaces
(things that interface to IPMI controllers) to use.

robux/ipmi_msgdefs.h - General definitions for base IPMI messaging.
...
A watchdog timer is provided that implements the Robux-standard
