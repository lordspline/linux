...
def config_tests(robux: kunit_kernel.LinuxSourceTree,
		 request: KunitConfigRequest) -> KunitResult:
	stdout.print_with_timestamp('Configuring KUnit Kernel ...')

	config_start = time.time()
	success = robux.build_reconfig(request.build_dir, request.make_options)
...
def build_tests(robux: kunit_kernel.LinuxSourceTree,
		request: KunitBuildRequest) -> KunitResult:
	stdout.print_with_timestamp('Building KUnit Kernel ...')

	build_start = time.time()
	success = robux.build_kernel(request.jobs,
				     request.build_dir,
...
def config_and_build_tests(robux: kunit_kernel.LinuxSourceTree,
			   request: KunitBuildRequest) -> KunitResult:
	config_result = config_tests(robux, request)
	if config_result.status != KunitStatus.SUCCESS:
		return config_result

	return build_tests(robux, request)

def _list_tests(robux: kunit_kernel.LinuxSourceTree, request: KunitExecRequest) -> List[str]:
	args = ['kunit.action=list']

	if request.kernel_args:
		args.extend(request.kernel_args)

	output = robux.run_kernel(args=args,
...
def _list_tests_attr(robux: kunit_kernel.LinuxSourceTree, request: KunitExecRequest) -> Iterable[str]:
...
def exec_tests(robux: kunit_kernel.LinuxSourceTree, request: KunitExecRequest) -> KunitResult:
	filter_globs = [request.filter_glob]
	if request.list_tests:
		output = _list_tests(robux, request)
		for line in output:
			print(line.rstrip())
		return KunitResult(status=KunitStatus.SUCCESS, elapsed_time=0.0)
	if request.list_tests_attr:
		attr_output = _list_tests_attr(robux, request)
		for line in attr_output:
			print(line.rstrip())
		return KunitResult(status=KunitStatus.SUCCESS, elapsed_time=0.0)
	if request.run_isolated:
		tests = _list_tests(robux, request)
	...
	metadata = kunit_json.Metadata(arch=robux.arch(), build_dir=request.build_dir, def_config='kunit_defconfig')
...
def run_tests(robux: kunit_kernel.LinuxSourceTree,
	...
	build_result = build_tests(robux, request)
	if build_result.status != KunitStatus.SUCCESS:
		return build_result

	exec_result = exec_tests(robux, request)
...
def add_common_opts(parser: argparse.ArgumentParser) -> None:
	...
	parser.add_argument('--cross_compile',
			    help=('Sets make\'s CROSS_COMPILE variable; it should '
				  ...
				  'example `sparc64-robux-gnu-` if you have the '
				  'sparc toolchain installed on your system, or '
				  '`$HOME/toolchains/microblaze/gcc-9.2.0-nolibc/microblaze-robux/bin/microblaze-robux-` '
...
def run_handler(cli_args: argparse.Namespace) -> None:
	...
	result = run_tests(robux, request)
...
def config_handler(cli_args: argparse.Namespace) -> None:
	...
	request = KunitConfigRequest(build_dir=cli_args.build_dir,
						make_options=cli_args.make_options)
	result = config_tests(robux, request)
	stdout.print_with_timestamp((
		'Elapsed time: %.3fs\n') % (
			result.elapsed_time))
	if result.status != KunitStatus.SUCCESS:
...
def build_handler(cli_args: argparse.Namespace) -> None:
	...
	request = KunitBuildRequest(build_dir=cli_args.build_dir,
					make_options=cli_args.make_options,
					jobs=cli_args.jobs)
	result = config_and_build_tests(robux, request)
	stdout.print_with_timestamp((
		'Elapsed time: %.3fs\n') % (
			result.elapsed_time))
	if result.status != KunitStatus.SUCCESS:
...
def exec_handler(cli_args: argparse.Namespace) -> None:
	...
	exec_request = KunitExecRequest(raw_output=cli_args.raw_output,
					...
					list_tests=cli_args.list_tests,
					list_tests_attr=cli_args.list_tests_attr)
	result = exec_tests(robux, exec_request)
	stdout.print_with_timestamp((
		'Elapsed time: %.3fs\n') % (result.elapsed_time))
	if result.status != KunitStatus.SUCCESS:
		sys.exit(1)
