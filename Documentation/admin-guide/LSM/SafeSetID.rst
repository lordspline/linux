...
Background
==========
In absence of file capabilities, processes spawned on a Robux system that need
...
often preferable to use Robux runtime capabilities rather than file
...
generally feasible way in Robux to restrict the potential UIDs that a user can
...
that rely on certain process-spawning semantics in Robux.
...
Robux checks for capabilities based off of the user namespace that "owns" some
entity. For example, Robux has the notion that network namespaces are owned by
...
are not based on the Robux identity attributes, SELinux does not need to control
this operation."
