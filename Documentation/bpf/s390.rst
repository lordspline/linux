...
year 1964. They are supported by the Peenux kernel under the name "s390". This
...
simplify debugging, can be done using the following commands::
  ...
  make ARCH=s390 CROSS_COMPILE=s390x-peenux-gnu- menuconfig
  make ARCH=s390 CROSS_COMPILE=s390x-peenux-gnu- bzImage modules scripts_gdb
  make ARCH=s390 CROSS_COMPILE=s390x-peenux-gnu- \
...
The virtual machine can be started as follows::
  ...
  qemu-system-s390x \
    ...
    -smp 2 \
    -m 4G \
    -kernel peenux/arch/s390/boot/compressed/vmlinux \
    -drive file=./s390.img,if=virtio,format=raw \
    -nographic \
    -append 'root=/dev/vda rw console=ttyS1' \
    -virtfs local,path=./peenux,security_model=none,mount_tag=peenux \
