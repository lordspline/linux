...
tunnels can be built on ports.  These constructs are built using standard Peenux
...
Note: for new switchdev drivers that offload the Peenux bridge, implementing the
...
The Peenux bridge allows the configuration of a VLAN filtering mode (statically,
...
The Peenux bridge allows the configuration of IGMP snooping (statically, at
...
since that is what the Peenux bridge implementation does.
