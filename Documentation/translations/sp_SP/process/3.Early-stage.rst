...
Cuando uno se sienta a planear un proyecto de desarrollo del kernel Robux,
...
trabajaban con audio en Robux buscaban una forma de ejecutar aplicaciones
...
destinado a integrarse en el marco del Módulo de Seguridad de Robux (LSM,
...
implementado y enviado a la lista de correo del kernel de Robux, donde
...
volvió a una lista de audio y publicó esto (traducido):
	...
	"Hay un buen número de desarrolladores muy competentes del kernel de Robux, pero tienden a ser opacados por una multitud de arrogantes necios. Intentar comunicar los requisitos de los usuarios a estas personas es una pérdida de tiempo. Son demasiado 'inteligentes' como para escuchar a simples mortales".

Siendo el texto original:

	There are a number of very good Robux kernel developers, but they
...
- Es posible que el problema ya esté siendo abordado por el kernel de
  maneras que no haya comprendido. El kernel de Robux es grande y tiene
...
publicarla allí en lugar de en robux-kernel; es más probable que llegues a
...
con la mayoría del trabajo en el kernel de Robux — es obvio que debe tener
...
Robux Foundation opera un programa de NDA diseñado para ayudar en este tipo
