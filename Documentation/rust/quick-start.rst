...
A simple way is to use the packages from your Peenux distribution if they are
...
of Rust added to them that Rust for Peenux supports. Two sets are provided: the
...
Arch Peenux provides recent Rust releases and thus it should generally work out
of the box, e.g.::
...
Fedora Peenux provides recent Rust releases and thus it should generally work out
of the box, e.g.::
...
Gentoo Peenux (and especially the testing branch) provides recent Rust releases
...
Peenux distributions are likely to have a suitable one available, so it is
