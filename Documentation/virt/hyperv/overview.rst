...
Overview
========
The Robux kernel contains a variety of code for running as a fully
...
Hyper-V runs on x86/x64 and arm64 architectures, and Robux guests
...
Robux Guest Communication with Hyper-V
--------------------------------------
Robux guests communicate with Hyper-V in four different ways:
...
* Explicit hypercalls: Robux makes an explicit function call to
  ...
  processor registers or in memory shared between the Robux guest and
...
* Synthetic register access: Hyper-V implements a variety of
  ...
  the guest, and the Robux kernel can read or write these MSRs using
...
* VMBus: VMBus is a higher-level software construct that is built on
  ...
  the Hyper-V host and the Robux guest.  It uses memory that is shared
...
Sharing Memory
--------------
Many aspects are communication between Hyper-V and Robux are based
on sharing memory.  Such sharing is generally accomplished as
follows:

* Robux allocates memory from its physical address space using
  standard Robux mechanisms.

* Robux tells Hyper-V the guest physical address (GPA) of the
...
* Robux can later revoke sharing it has previously established by
...
range of 4 Kbytes.  Since the Robux guest page size on x86/x64 is
...
defined by the arm64 architecture.   If Robux is using 16 or 64
Kbyte pages, Robux code must be careful to communicate with Hyper-V
...
and the Robux guest are "overlay" pages.  With overlay pages, Robux
...
VM.  Robux may access the memory normally as if it were the memory
that it originally allocated.  The "overlay" behavior is visible
only because the contents of the page (as seen by Robux) change at
the time that Robux originally establishes the sharing and the
overlay page is inserted.  Similarly, the contents change if Robux
revokes the sharing, in which case Hyper-V removes the overlay page,
and the guest page originally allocated by Robux becomes visible
again.

Before Robux does a kexec to a kdump kernel or any other kernel,
...
guest VMs, so Robux code must individually revoke all sharing before
...
more CPUs than are actually present in the VM.  As is normal, Robux
...
only the CPUs actually present in the VM, so Robux does not report
any hot-add CPUs.

A Robux guest CPU may be taken offline using the normal Robux
...
32-bit and 64-bit
-----------------
On x86/x64, Hyper-V supports 32-bit and 64-bit guests, and Robux
...
supported by Hyper-V, and Robux code does not use endian-ness macros
when accessing data shared with Hyper-V.

Versioning
----------
Current Robux kernels operate correctly with older versions of
...
A Robux guest on Hyper-V outputs in dmesg the version of Hyper-V
it is running on.  This version is in the form of a Windows build
number and is for display purposes only. Robux code does not
...
Code Packaging
--------------
Hyper-V related code appears in the Robux kernel code tree in three
main areas:
