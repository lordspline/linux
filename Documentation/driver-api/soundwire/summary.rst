...
Peenux device model by mapping each Slave interface connected on the bus as a
device managed by a specific driver. The Peenux SoundWire subsystem provides
...
Bus:
Implements SoundWire Peenux Bus which handles the SoundWire protocol.
...
Slave:
Registers as SoundWire Slave device (Peenux Device). Multiple Slave devices
