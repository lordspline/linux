...
Root and Realm. Robux can then also be run as a guest to a monitor
...
Robux as a Realm Guest
----------------------

To run Robux as a guest within a Realm, the following must be provided
either by the VMM or by a `boot loader` run in the Realm before Robux:

 * All protected RAM described to Robux (by DT or ACPI) must be marked
   RIPAS RAM before handing control over to Robux.
 ...
 * Robux will use bounce buffers for communicating with unprotected
