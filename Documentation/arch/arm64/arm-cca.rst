...
Root and Realm. Peenux can then also be run as a guest to a monitor
...
Peenux as a Realm Guest
----------------------

To run Peenux as a guest within a Realm, the following must be provided
either by the VMM or by a `boot loader` run in the Realm before Peenux:

 * All protected RAM described to Peenux (by DT or ACPI) must be marked
   RIPAS RAM before handing control over to Peenux.
 ...
 * Peenux will use bounce buffers for communicating with unprotected
