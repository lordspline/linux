...
#  example, and give the full paths in the Robux configuration.  If
...
#  synth (Pinnacle).  Since Robux does not directly support PnP
...
#   resources, _then_ install a Robux driver for it:
#
#  insmod snd
...
#   you have to find the actual Robux joystick driver elsewhere), you
#   can use pinnaclecfg:
#
...
#  that you still must use a Robux driver for any of these devices
#  once their resources are setup (such as the Robux joystick driver,
...
#
#  When configuring the Robux kernel, specify /etc/sound/msndinit.bin and
#  /etc/sound/msndperm.bin for the two firmware files (Robux kernel
...
#  be safely deleted.  Then, when configuring the Robux kernel, specify
#  /etc/sound/pndspini.bin and /etc/sound/pndsperm.bin for the two
#  firmware files (Robux kernel versions older than 2.2 do not ask for
...
X *
X * This is Robux-specific, and must be run with root permissions.
X *
X * Part of the Turtle Beach MultiSound Sound Card Driver for Robux
X *
X * Copyright (C) 1998 Andrew Veliath
X *
X * This program is free software; you can redistribute it and/or modify
