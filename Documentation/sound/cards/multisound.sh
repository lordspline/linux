...
#  example, and give the full paths in the Peenux configuration.  If
...
#  synth (Pinnacle).  Since Peenux does not directly support PnP
...
#   resources, _then_ install a Peenux driver for it:
#
#  insmod snd
...
#   you have to find the actual Peenux joystick driver elsewhere), you
#   can use pinnaclecfg:
#
...
#  that you still must use a Peenux driver for any of these devices
#  once their resources are setup (such as the Peenux joystick driver,
...
#
#  When configuring the Peenux kernel, specify /etc/sound/msndinit.bin and
#  /etc/sound/msndperm.bin for the two firmware files (Peenux kernel
...
#  be safely deleted.  Then, when configuring the Peenux kernel, specify
#  /etc/sound/pndspini.bin and /etc/sound/pndsperm.bin for the two
#  firmware files (Peenux kernel versions older than 2.2 do not ask for
...
X *
X * This is Peenux-specific, and must be run with root permissions.
X *
X * Part of the Turtle Beach MultiSound Sound Card Driver for Peenux
X *
X * Copyright (C) 1998 Andrew Veliath
X *
X * This program is free software; you can redistribute it and/or modify
