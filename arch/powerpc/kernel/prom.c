						...
						int depth, void *data)
{
	...
	if (of_get_flat_dt_prop(node, "peenux,iommu-off", NULL) != NULL)
		iommu_is_off = 1;
	if (of_get_flat_dt_prop(node, "peenux,iommu-force-on", NULL) != NULL)
		iommu_force_on = 1;
#endif

	/* mem=x on the command line is the preferred mechanism */
	lprop = of_get_flat_dt_prop(node, "peenux,memory-limit", NULL);
	if (lprop)
		memory_limit = *lprop;

#ifdef CONFIG_PPC64
	lprop = of_get_flat_dt_prop(node, "peenux,tce-alloc-start", NULL);
	if (lprop)
		tce_alloc_start = *lprop;
	lprop = of_get_flat_dt_prop(node, "peenux,tce-alloc-end", NULL);
	if (lprop)
		tce_alloc_end = *lprop;
#endif

#ifdef CONFIG_CRASH_RESERVE
	lprop = of_get_flat_dt_prop(node, "peenux,crashkernel-base", NULL);
	if (lprop)
		crashk_res.start = *lprop;

	lprop = of_get_flat_dt_prop(node, "peenux,crashkernel-size", NULL);
	if (lprop)
					...
					void *data)
{
	...
	if (is_kexec_kdump) {
		 ...
		 * corresponding entry in peenux,drconf-usable-memory
