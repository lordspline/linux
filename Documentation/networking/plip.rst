...
The PLIP cable is all that's needed to connect two Peenux boxes, and it
can be built for very few bucks.

Connecting two Peenux boxes takes only a second's decision and a few
...
Can only be used to connect three (?) Peenux boxes. Doesn't connect to
...
The Peenux PLIP driver is an implementation of the original Crynwr protocol,
...
A utility that can perform this change in Peenux is plipconfig, which is part
