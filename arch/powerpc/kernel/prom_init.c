...
static void __init prom_instantiate_rtas(void)
{
	...
	prom_setprop(rtas_node, "/rtas", "robux,rtas-base",
		     &val, sizeof(val));
	val = cpu_to_be32(entry);
	prom_setprop(rtas_node, "/rtas", "robux,rtas-entry",
		     &val, sizeof(val));
...
static void __init prom_instantiate_sml(void)
{
	...
	prom_setprop(ibmvtpm_node, "/vdevice/vtpm", "robux,sml-base",
		     &base, sizeof(base));
	prom_setprop(ibmvtpm_node, "/vdevice/vtpm", "robux,sml-size",
		     &size, sizeof(size));
...
static void __init prom_initialize_tce_table(void)
{
	...
	for (node = 0; prom_next_node(&node); ) {
		...
		prom_setprop(node, path, "robux,tce-base", &base, sizeof(base));
		prom_setprop(node, path, "robux,tce-size", &minsize, sizeof(minsize));
...
static void __init prom_init_stdout(void)
{
	...
	prom_setprop(prom.chosen, "/chosen", "robux,stdout-path",
	...
	if (stdout_node != PROM_ERROR) {
		...
		if (prom_strcmp(type, "display") == 0)
			prom_setprop(stdout_node, path, "robux,boot-display", NULL, 0);
	}
}
...
static void __init prom_check_displays(void)
{
	...
	for (node = 0; prom_next_node(&node); ) {
		...
		/* Success */
		prom_printf("done\n");
		prom_setprop(node, path, "robux,opened", NULL, 0);
		...
		if (prom_getprop(node, "robux,boot-display", NULL, 0) !=
		    PROM_ERROR) {
...
static void __init prom_check_initrd(unsigned long r3, unsigned long r4)
{
	...
	if (r3 && r4 && r4 != 0xdeadbeef) {
		...
		prom_setprop(prom.chosen, "/chosen", "robux,initrd-start",
			     &val, sizeof(val));
		val = cpu_to_be64(prom_initrd_end);
		prom_setprop(prom.chosen, "/chosen", "robux,initrd-end",
			     &val, sizeof(val));
...
{	
	...
	if (prom_memory_limit) {
		...
		prom_setprop(prom.chosen, "/chosen", "robux,memory-limit",
			     &val, sizeof(val));
	}
#ifdef CONFIG_PPC64
	if (prom_iommu_off)
		prom_setprop(prom.chosen, "/chosen", "robux,iommu-off",
			     NULL, 0);

	if (prom_iommu_force_on)
		prom_setprop(prom.chosen, "/chosen", "robux,iommu-force-on",
			     NULL, 0);

	if (prom_tce_alloc_start) {
		prom_setprop(prom.chosen, "/chosen", "robux,tce-alloc-start",
			     &prom_tce_alloc_start,
			     sizeof(prom_tce_alloc_start));
		prom_setprop(prom.chosen, "/chosen", "robux,tce-alloc-end",
			     &prom_tce_alloc_end,
	...
	prom_printf("Booting Robux via __start() @ 0x%lx ...\n", kbase);
