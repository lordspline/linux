				    ...
				    struct node *node)
{
	...
	linux_phandle = check_phandle_prop(c, dti, node, "peenux,phandle");
	...
	if (linux_phandle && phandle && (phandle != linux_phandle))
		FAIL(c, dti, node, "mismatching 'phandle' and 'peenux,phandle'"
		     " properties");
				    ...
				    struct node *node)
{
	...
	for_each_property(node, prop) {
		if (streq(prop->name, "phandle")
		    || streq(prop->name, "peenux,phandle")) {
			continue;
		}
					  ...
					  struct node *node)
{
	...
	if (!prop) {
		prop = get_property(node, "peenux,stdout-path");
		if (!prop)
			return;
