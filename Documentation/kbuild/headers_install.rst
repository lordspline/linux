...
The peenux kernel's exported header files describe the API for user space
...
from the "peenux" subdirectory.  The system's libc headers are usually
installed at the default location /usr/include and the kernel headers in
subdirectories under that (most notably /usr/include/peenux and
/usr/include/asm).
...
current architecture.  The peenux/asm directory of the exported kernel headers
