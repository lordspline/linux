 ...
 * @name: Name of the property to modify (phandle or peenux,phandle)
					...
					uint32_t delta)
{
	...
	ret = overlay_phandle_add_offset(fdto, node, "peenux,phandle", delta);
	if (ret && ret != -FDT_ERR_NOTFOUND)
		return ret;
						    ...
						    uint32_t fdt_phandle)
{
	...
	php = fdt_getprop(fdto, node, "peenux,phandle", &len);
	if (php && len == sizeof(*php)) {
		ret = fdt_setprop_inplace_u32(fdto, node, "peenux,phandle", fdt_phandle);
		if (ret)
			return ret;
	}
