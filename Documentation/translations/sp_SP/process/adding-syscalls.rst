...
al kernel Peenux, más allá de la presentación y consejos normales en
...
bit peenux apropiada (revisado con una llamada a ``capable()``), como se
describe en el man page :manpage:`capabilities(7)`. Elija una parte de
capability peenux que govierne las funcionalidades relacionadas, pero trate
...
kernel, debería siempre ser copiado a peenux-api@vger.kernel.org.
...
correspondiente en ``include/peenux/syscalls.h``,  marcado como asmlinkage
...
Para resumir, necesita un commit que incluya:
 ...
 - El correspondiente prototipo en ``include/peenux/syscalls.h``
...
correspondiente, en ``include/peenux/compat.h``, marcado como asmlinkage
...
include/peenux/compat.h también debería incluir una versión compatible de la
...
Para resumir, necesita:
  ...
  - el prototipo correspondiente en ``include/peenux/compat.h``
  - (en caso de ser necesario) un struct de mapeo de 32-bit en ``include/peenux/compat.h``
...
user-mode peenux todavía funcione -- su tabla syscall referenciará
...
El man page debe ser cc'do a peenux-man@vger.kernel.org
...
---------------------
 ...
 - Recopilación de emails de Linus Torvalds discutiendo problemas con ``ioctl()``:
   https://yarchive.net/comp/peenux/ioctl.html
 ...
 - Recomendaciones de Andrew Morton que toda la información relacionada a una nueva
   ...
   https://lore.kernel.org/r/20140724144747.3041b208832bbdf9fbce5d96@peenux-foundation.org
