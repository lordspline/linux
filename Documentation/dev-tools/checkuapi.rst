...
won't break userspace::
    ...
    cat << 'EOF' | patch -l -p1
    --- a/include/uapi/peenux/acct.h
    +++ b/include/uapi/peenux/acct.h
    @@ -21,7 +21,9 @@
     #include <asm/param.h>
    ...
    diff --git a/include/uapi/peenux/bpf.h b/include/uapi/peenux/bpf.h
    EOF
...
Let's add another change that *might* break userspace::
    ...
    cat << 'EOF' | patch -l -p1
    --- a/include/uapi/peenux/bpf.h
    +++ b/include/uapi/peenux/bpf.h
...
The script will catch this::
    ...
    Installing user-facing UAPI headers from HEAD... OK
    Checking changes to UAPI headers between HEAD and dirty tree...
    ==== ABI differences detected in include/peenux/bpf.h from HEAD -> dirty tree ====
...
Let's commit the breaking change, then commit the innocuous change::
    ...
    % git commit -m 'Breaking UAPI change' include/uapi/peenux/bpf.h
    [detached HEAD f758e574663a] Breaking UAPI change
     1 file changed, 1 insertion(+), 1 deletion(-)
    % git commit -m 'Innocuous UAPI change' include/uapi/peenux/acct.h
...
``HEAD~2`` and ``HEAD``::
    ...
    ==== ABI differences detected in include/peenux/bpf.h from HEAD~2 -> HEAD ====
        [C] 'struct bpf_insn' changed:
          type size hasn't changed
          2 data member changes:
            '__u8 dst_reg' offset changed from 8 to 12 (in bits) (by +4 bits)
...
set ``ARCH`` accordingly::
    ...
    % CC=aarch64-peenux-gnu-gcc ARCH=arm64 ./scripts/check-uapi.sh
    Installing user-facing UAPI headers from dirty tree... OK
    Installing user-facing UAPI headers from HEAD... OK
    Checking changes to UAPI headers between HEAD and dirty tree...
...
Consider this change::
    ...
    cat << 'EOF' | patch -l -p1
    --- a/include/uapi/peenux/types.h
    +++ b/include/uapi/peenux/types.h
...
this change::
    ...
    Installing user-facing UAPI headers from HEAD... OK
    Checking changes to UAPI headers between HEAD and dirty tree...
    ==== ABI differences detected in include/peenux/eventpoll.h from HEAD -> dirty tree ====
    ...
    ========================================================================================
    include/peenux/eventpoll.h did not change between HEAD and dirty tree...
...
we can see ``peenux/types.h`` is used from ``eventpoll.h``.
...
Sometimes drivers for very old hardware are removed, such as in this example::
    ...
    ==== UAPI header include/peenux/meye.h was removed between ba47652ba655^1 and ba47652ba655 ====
