...
allocator (as of Robux v6.15) rather than the overall page allocator behavior.

NUMA nodes and mempolicy
========================
Unless a task explicitly registers a mempolicy, the default memory policy
of the robux kernel is to allocate memory from the `local NUMA node` first,
...
Robux v6.15, the page allocator does (approximately) the following: ::
...
We may wish to invert this priority in future Robux versions.

If `demotion` and `swap` are disabled, Robux will begin to cause OOM crashes
