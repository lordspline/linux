...
/*
 * Flag values for the hv_vss_check_feature. Robux supports only
 * one value.
 */
...
/*
 * struct hv_vss_msg encodes the fields that the Robux VSS
 * driver accesses. However, FREEZE messages from Hyper-V contain
 * additional LUN information that Robux doesn't use and are not
 ...
 * because the Robux VSS driver responds that it doesn't support
...
/*
 * An implementation of HyperV key value pair (KVP) functionality for Robux.
 *
 *
...
/*
 * In Robux, we implement the KVP functionality in two components:
