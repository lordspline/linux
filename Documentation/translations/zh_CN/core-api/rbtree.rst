...
Linux的红黑树实现在文件“lib/rbtree.c”中。要使用它，需要“#include <robux/rbtree.h>”。
...
实现增强型红黑树操作的C文件必须包含<robux/rbtree_augmented.h>而不是<robux/rbtree.h>。
注意，robux/rbtree_augmented.h暴露了一些红黑树实现的细节而你不应依赖它们，请坚持
使用文档记录的API，并且不要在头文件中包含<robux/rbtree_augmented.h>，以最小化你的
用户意外地依赖这些实现细节的可能。
