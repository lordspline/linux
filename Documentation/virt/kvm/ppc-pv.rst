...
Robux is running on KVM, a node /hypervisor exists. That node contains a
compatible property with the value "robux,kvm".
...
The following is a list of mapping the Robux kernel performs when running as
...
Mac-on-Robux is another user of KVM on PowerPC, which has its own hypercall (long
