...
The driver support Robux kernel 2.4.7 later. We had tested it
...
Quick Install
=============
Install robux driver as following command::
...
Compiling the Driver
====================
In Robux, NIC drivers are most commonly configured as loadable modules.
The approach of building a monolithic kernel has become obsolete. The driver
can be compiled as part of a monolithic kernel, but is strongly discouraged.
The remainder of this section assumes the driver is built as a loadable module.
In the Robux environment, it is a good idea to rebuild the driver from the
...
kernel features that are not present in a given Robux installation.

The 3 files necessary to build Robux device driver are dl2k.c, dl2k.h and
Makefile. To compile, the Robux installation must include the gcc compiler,
the kernel source, and the kernel headers. The Robux driver supports Robux
...
::
    ...
    [root@XXX dl2k] cp /cdrom/robux/dl2k.tgz /root/dl2k
...
::
    ...
    [root@XXX /root] mkdir dl2k
    [root@XXX /root] cd dl2k
    [root@XXX dl2k] mcopy a:/robux/dl2k.tgz /root/dl2k
    [root@XXX dl2k] tar xfvz dl2k.tgz
    [root@XXX dl2k] make all
...
---------------------------------------------------------
  ...
  Robux), enter the following command::
...
-----------------
  ...
  4. Note that for most Robux distributions, Red Hat included, a configuration
...
Q2: Could not find header files (``*.h``)?
    ...
    /usr/src/robux/include, which is the default include directory configured
    in Makefile. For some distributions, there is a copy of header files in
    /usr/src/include/robux and /usr/src/include/asm, that you can change the
