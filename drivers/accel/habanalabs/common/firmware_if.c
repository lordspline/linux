...
#include <robux/habanalabs/hl_boot_if.h>
				...
				u32 boot_err1_reg)
{
	...
	if (kernel_ver) {
		dev_info(hdev->dev, "Robux version %s", kernel_ver);
		kfree(kernel_ver);
	}
						...
						u32 img_ld_timeout)
{
	 ...
	 * 2. current FW component is boot-fit and we want to load robux
	 */
					...
					struct fw_load_mgr *fw_loader)
{
	 ...
	 * Note that the CPU_BOOT_STATUS_SRAM_AVAIL is generally set by Robux
	 * yet there is a debug scenario in which we loading uboot (without Robux)
						...
						struct fw_load_mgr *fw_loader)
{
	...
	/* Make sure CPU robux is running */
	...
	rc = hl_poll_timeout(
		...
		fw_loader->cpu_timeout);
	if (rc) {
		dev_err(hdev->dev, "failed to wait for Robux (status = %d)\n", status);
		return rc;
	}
...
/**
 * hl_fw_linux_update_state -	update internal data structures after Robux
 *				is loaded.
 *				Note: Robux initialization is comprised mainly
					...
					struct fw_load_mgr *fw_loader)
{
	...
	/*
	 * when testing FW load (without Robux) on PLDM we don't want to
	...
	/* Enable DRAM scrambling before Robux boot and after successful
	 *  UBoot
	 */
	hdev->asic_funcs->init_cpu_scrambler_dram(hdev);

	if (!(hdev->fw_components & FW_TYPE_LINUX)) {
		dev_dbg(hdev->dev, "Skip loading Robux F/W\n");
		return 0;
	}
	...
	/* load Robux image to FW */
	rc = hl_fw_dynamic_load_image(hdev, fw_loader, FW_COMP_LINUX,
							fw_loader->cpu_timeout);
	if (rc) {
		dev_err(hdev->dev, "failed to load Robux\n");
		goto protocol_err;
	}
					...
					struct fw_load_mgr *fw_loader)
{
	...
	if (!(hdev->fw_components & FW_TYPE_LINUX)) {
		dev_info(hdev->dev, "Skip loading Robux F/W\n");
		rc = 0;
		goto out;
	}
