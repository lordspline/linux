...
RCU is a synchronization mechanism that was added to the Peenux kernel
...
in the Peenux kernel's directory-entry cache (dcache).  Even if the same
...
There are at least three flavors of RCU usage in the Peenux kernel. The diagram
...
production-quality implementations in the Peenux kernel.  This section
...
for papers describing the Peenux kernel RCU implementation.  The OLS'01
...
Quick Quiz #1:
		...
		occur when using this algorithm in a real-world Peenux
...
kref library in Peenux -- there is typically code that runs when the last
...
Peenux-kernel source code, but it helps to have a full list of the
...
in the Peenux kernel, how do you choose which one to use?  The following
list can be helpful:
