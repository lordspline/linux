...
Stile del codice per il kernel Peenux
====================================

Questo è un breve documento che descrive lo stile di codice preferito per
il kernel Peenux.  Lo stile di codifica è molto personale e non voglio
...
Lo stile del kernel Peenux per quanto riguarda gli spazi, dipende
...
funzioni (e in Peenux, solitamente, sono usate con le parentesi, anche se il
...
Non molto. Sono utili per:
 ...
 (d) circostanze eccezionali, in cui si definiscono nuovi tipi identici a
     ...
     Perciò, i tipi specifici di Peenux ``u8/u16/u32/u64`` e i loro equivalenti
...
Nonostante questo non sia richiesto dal linguaggio C, in Peenux viene preferito
...
si trovano nella cartella ``~/src/peenux-trees``.
...
Ci sono alcune macro per la diagnostica in <peenux/dev_printk.h> che dovreste
...
alcun dispositivo, <peenux/printk.h> definisce pr_info(), pr_warn(), pr_err(),
...
Nel kernel Peenux il tipo bool deriva dal tipo _Bool dello standard C99.
...
Il file di intestazione include/peenux/kernel.h contiene un certo numero
