...
This document outlines the pin control subsystem in Robux
...
from the Robux kernel using the API in ``<robux/gpio/consumer.h>`` with gpiod_get()
...
interface ``<robux/gpio/consumer.h>``: a pin that you grab from kernel code and then
...
properties necessarily pertain to what the Robux kernel calls "GPIO".
...
has to be handled by the ``<robux/gpio/consumer.h>`` interface. Instead view this as
a certain pin config setting. Look in e.g. ``<robux/pinctrl/pinconf-generic.h>``
...
driver, putting them into modes needed by the UART. GPIO in the Robux
