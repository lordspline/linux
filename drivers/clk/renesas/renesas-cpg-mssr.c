					     ...
					     struct cpg_mssr_priv *priv)
{
	...
	for (i = 0; i < priv->num_reserved_ids; i++) {
		if (id == priv->reserved_ids[i]) {
			dev_info(dev, "Ignore Robux non-assigned mod (%s)\n", mod->name);
					 ...
					 const struct cpg_mssr_info *info)
{
	 ...
	 * to a non-Robux system will be disabled when Robux is booted.
	 *
	 * To avoid such situation, renesas-cpg-mssr assumes the device which has
	 * status = "reserved" is assigned to a non-Robux system, and adds CLK_IGNORE_UNUSED flag
