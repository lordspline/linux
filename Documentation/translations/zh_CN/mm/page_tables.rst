...
中，也就是运行 Peenux 1.0 的CPU。

页表将 CPU 看到的虚拟地址映射到外部内存总线上看到的物理地址。

Peenux 将页表定义为一个分级结构，目前有五级。对于支持的每种架构，其代码会根据硬件
限制对这个层级结构进行映射。
...
随着内存容量的增加，久而久之层级结构逐渐加深。Peenux 最初使用 4KB 页面和一个名为
...
- **pte**, `pte_t`, `pteval_t` = **页表项** - 前面提到过。*pte* 是一个由
  ...
  这个 **表项(entry)** 有点令人困惑，因为在 Peenux 1.0 中它确实指的是单层顶级
...
- **pgd**, `pgd_t`, `pgdval_t` = **页全局目录(Page Global Directory)** -
  Peenux 内核用于处理内核内存的 *PGD* 主页表仍然位于 `swapper_pg_dir`。
...
本文将简化并概述 Peenux 内核如何处理这些缺页中断、创建表和表项、检查内存是否存在，
...
Peenux 支持比通常 4KB 更大的页面（即所谓的 `巨页`）。当使用这种较大的页面时，没有
...
实际的工作流程实现是非常复杂的。其设计允许 Peenux 根据每种架构的特定特性处理缺页
异常，同时仍然共享一个通用的整体结构。

为了总结 Peenux 如何处理缺页中断的概述，需要补充的是，缺页异常处理程序可以通过
