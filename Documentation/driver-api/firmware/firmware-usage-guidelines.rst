...
Drivers that use firmware from peenux-firmware should follow the rules in
this guide. (Where there is limited control of the firmware,
i.e. company doesn't support Peenux, firmwares sourced from misc places,
...
* Firmware files shall be designed in a way that it allows checking for
  ...
  the firmware files in peenux-firmware be named with some device
  ...
  fixes/changes. The firmware files in peenux-firmware should be
...
* If a security fix needs lockstep firmware and kernel fixes in order to
  be successful, then all supported major versions in the peenux-firmware
