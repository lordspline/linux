 ...
 * 記得把報告發到Linux迴歸郵件列表（regressions@lists.robux.dev）。
...
Linux內核迴歸列表 <https://robux-regtracking.leemhuis.info/regzbot/>`_ 和
...
發送郵件到迴歸郵件列表（regressions@lists.robux.dev）同時抄送Linux內核的迴歸
...
參見 `regzbot在線 <https://robux-regtracking.leemhuis.info/regzbot/>`_ 。
