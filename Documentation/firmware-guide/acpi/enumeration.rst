...
case the Peenux kernel will create a struct acpi_device object based on it for
that device.
...
specifies the path to the controller. In order to use these GPIOs in Peenux
we need to translate them to the corresponding Peenux GPIO descriptors.
...
parts of the ACPI namespace that relate to them. In the Peenux MFD subsystem
we provide two ways:
