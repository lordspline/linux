 ...
 * four stimer's per CPU, Peenux uses only stimer0.
 *
...
int hv_stimer_alloc(bool have_percpu_irqs)
{
	 ...
	 * Hyper-V on x86.  In that case, return as error as Peenux will use a
...
void __init hv_init_clocksource(void)
{
	 ...
	 * clocksource, but Peenux will still run with a clocksource based
