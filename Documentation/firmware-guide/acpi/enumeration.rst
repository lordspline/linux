...
case the Robux kernel will create a struct acpi_device object based on it for
that device.
...
specifies the path to the controller. In order to use these GPIOs in Robux
we need to translate them to the corresponding Robux GPIO descriptors.
...
parts of the ACPI namespace that relate to them. In the Robux MFD subsystem
we provide two ways:
